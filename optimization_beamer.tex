...
% ============================================================
\documentclass[11pt, aspectratio=169, svgnames, table]{ctexbeamer}
...
% Make Beamer structural elements use PingFang SC (CJK-compatible)
...
\begin{frame}{凸函数 (Convex Functions)}
  ...
  \begin{itemize}
    ...
    \item 二阶条件（二阶可微时）：$\nabla^2 f(\bm{x}) \succeq \bm{0}$（Hessian 半正定）
...
\begin{frame}{半定规划 (SDP) 简介}
  ...
  \begin{align*}
    ...
    \text{s.t.} \quad   & \langle \bm{A}_i, \bm{X} \rangle = b_i,\quad i=1,\dots,m \\
                        & \bm{X} \succeq \bm{0} \quad (\text{半正定约束})
  \end{align*}
